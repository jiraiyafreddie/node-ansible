#!/usr/bin/env bash
INSTALL_DIR="/opt/shibarium"  #needs change
SHIBARIUM_ANSIBLE_REPO="https://github.com/shibaswaparmy/node-ansible.git"
...
function clone_repo() {
  git clone ${SHIBARIUM_ANSIBLE_REPO} ${INSTALL_DIR}
}
...
function configure_node() {
  ...
  if [ "${network}" == "mainnet" ];then
    sed -i 's/^seeds =.*/seeds ="f4f605d60b8ffaaf15240564e58a81103510631c@159.203.9.164:26656,4fb1bc820088764a564d4f66bba1963d47d82329@44.232.55.71:26656,2eadba4be3ce47ac8db0a3538cb923b57b41c927@35.199.4.13:26656,3b23b20017a6f348d329c102ddc0088f0a10a444@35.221.13.28:26656,25f5f65a09c56e9f1d2d90618aa70cd358aa68da@35.230.116.151:26656"/g' "${HEIMDALLD_CONFIG}"    #Heimdall seeds https://shibaswap.atlassian.net/wiki/spaces/TEAM/pages/7372806/Add+New+Validator+Using+Binaries#Configuring-the-Heimdall-services%E2%80%8B
  fi
  ...
  if ! grep -q bootnodes "${BOR_START_SH}"; then
    ...
    # bor enode config https://shibaswap.atlassian.net/wiki/spaces/TEAM/pages/7372806/Add+New+Validator+Using+Binaries#Configuring-the-Bor-service%E2%80%8B
  fi
...
if [ -z "${e}" ];then
  e="bor_branch=v0.2.14 heimdall_branch=v0.2.9-bone  network_version=alpha_v3 node_type=sentry/sentry heimdall_network=mainnet"
fi
echo "EXTRA_VAR=${e}"
